...
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    private void init() {
        ...
        // ExtentReports 5.x 使用 ExtentSparkReporter 替代 ExtentHtmlReporter
        ExtentSparkReporter sparkReporter = new ExtentSparkReporter(OUTPUT_FOLDER + FILE_NAME);
        sparkReporter.config().setDocumentTitle(ReportUtil.getReportName());
        sparkReporter.config().setReportName(ReportUtil.getReportName());
        sparkReporter.config().setTheme(Theme.STANDARD);
        // ExtentReports 5.x 中 CSS 和 JS 配置方式有所变化
        sparkReporter.config().setCss(".node.level-1  ul{ display:none;} .node.level-1.active ul{display:block;}  .card-panel.environment  th:first-child{ width:30%;}");
        sparkReporter.config().setJs("$(window).off(\"keydown\");");
    ...
    private void buildTestNodes(ExtentTest extenttest,IResultMap tests, Status status) {
        ...
        if(extenttest != null ){
            // ExtentReports 5.x 中，简化 category 处理，直接使用空数组
            // 因为 ExtentReports 5.x API 变化较大，这里采用简化处理
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                for(String output:outputList){
                    //将用例的log输出报告中，ExtentReports 5.x 使用 info 替代 debug
                    test.info(output.replaceAll("<","&lt;").replaceAll(">","&gt;"));
                }

...
public class ApiTest extends TestBase {
	...
	public void init(@Optional("api-config.xml") String envName) throws Exception {
		...
		// 注意：新版本的 HttpClient 使用 RequestConfig 而不是 getParams()
	}
...
//						.toString(respEntity));
	...
	private HttpEntity parseEntity(String param,boolean formData) throws UnsupportedEncodingException{
		if(formData){
			...
			Map<String, String> paramMap = JsonUtil.parseObject(param, HashMap.class);
			...
			for (String key : paramMap.keySet()) {
				...
				if (m.matches() && m.group(1).equals("bodyfile")) {
					...
					builder.addTextBody(key, paramMap.get(key), ContentType.TEXT_PLAIN);
				}
			}
